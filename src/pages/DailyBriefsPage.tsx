    ...
    <Card>
      ...
      <CardContent className="grid gap-4">
        <div className="grid gap-3 text-sm md:grid-cols-3">
          <Meta label="Brief date" value={formatDate(brief.brief_date)} />
          <Meta label="Period covered" value={`${formatDate(brief.period_start)} - ${formatDate(brief.period_end)}`} />
              ...
              <tbody className="divide-y">
                {briefs.map((brief) => (
                  <tr key={brief.id}>
                    ...
                    <td className="py-3 pr-3">{formatDate(brief.period_start)} - {formatDate(brief.period_end)}</td>
...
function formatBriefForClipboard(brief: AiDailyBrief) {
  return [
    `Daily Brief - ${formatDate(brief.brief_date)}`,
    `Period: ${formatDate(brief.period_start)} - ${formatDate(brief.period_end)}`,
    `Status: ${brief.status}`,
    `Model: ${brief.model}`,
    "",
    "Executive Summary",
